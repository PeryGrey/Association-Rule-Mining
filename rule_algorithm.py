...
class RuleBuilderAlgorithm:
    ...
    def calculate_default_class_properties(self, clf):
        ...
        clf.default_class_support = class_distribution[default_class] / len(
            self.y)
        clf.default_class_confidence = class_distribution[default_class] / len(
            self.y)

        default_rule_ant = Antecedent({})
        default_rule_conseq = Consequent(
            clf.default_class_attribute, clf.default_class)
