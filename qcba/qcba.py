...
class QCBA:
    ...
    def __init__(self, quantitative_dataset, cba_rule_model=None, rules=None):
        if rules and cba_rule_model:
            raise Exception(
                "rules and cba_rule_model cannot be specified together")

        if not rules and not cba_rule_model:
            raise Exception(
                "either rules and cba_rule_model need to be specified")

        self.quantitative_dataset = quantitative_dataset
        self.__quant_rules = None

        if cba_rule_model:
            self.__quant_rules = [QuantitativeCAR(
                r) for r in cba_rule_model.clf.rules]
            print(cba_rule_model.clf.rules)
        if rules:
    ...
    def fit(
        ...
        transformed_rules, default_class = self.qcba_transformation.transform(
            self.__quant_rules, transformation_dict)
